...
LV_IMG_DECLARE(my_weather);                                 /* 声明天气背景图片 */
...
static void lv_example_imgbtn(void)
{
    ...
    lv_obj_align(imgbtn_cool, LV_ALIGN_CENTER, scr_act_width()/3, 0);                /* 设置位置 */
    ...
    lv_obj_align(imgbtn_warm, LV_ALIGN_CENTER, 0, 0);                                 /* 设置位置 */
    ...
    lv_obj_align(imgbtn_dry, LV_ALIGN_CENTER, -scr_act_width()/3, 0);                  /* 设置位置 */
    ...
    lv_img_set_src(img, &my_weather);
    // 计算缩放比例
    uint16_t zoom_factor = 341; // 大约等于 1.333 * 256
